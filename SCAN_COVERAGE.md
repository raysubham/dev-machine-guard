...
- **Recorded skill use:** Claude's `skillUsage` keys, cumulative counts (including zero) and native millisecond timestamps. Counters survive missing or uninstalled definitions and are not summed or attributed to a current plugin by the collector.
