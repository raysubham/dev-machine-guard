...
func newSecureTestHome(t *testing.T, home string) *secureUserHome {
	t.Helper()
	u, err := user.Current()
	if err != nil {
	...
	h, err := openSecureUserHome(u)
	if err != nil {
		t.Fatalf("openSecureUserHome: %v", err)
	}
	t.Cleanup(func() { _ = h.Close() })
	return h
}

func openSecureTestFile(t *testing.T, h *secureUserHome, relativePath string) *secureUserFile {
	t.Helper()
	f, err := h.openStrict(relativePath, ".dmg-", maxManagedUserFileBytes)
	if err != nil {
...
func TestSecureUserFile_StrictMetadataIsOptIn(t *testing.T) {
	h := newSecureTestHome(t, t.TempDir())
	legacy, err := h.open(".npmrc", ".dmg-", maxManagedUserFileBytes)
	if err != nil {
		t.Fatal(err)
	}
	strict, err := h.openStrict(".netrc", ".dmg-", maxManagedUserFileBytes)
	if err != nil {
		t.Fatal(err)
	}
	if legacy.strictMetadata {
		t.Fatal("legacy npm file unexpectedly enables strict platform metadata")
	}
	if !strict.strictMetadata {
		t.Fatal("Python credential file did not enable strict platform metadata")
	}
}
...
func TestSecureUserFile_CreatesPinnedParentsAndCommits(t *testing.T) {
	tests := []struct {
		...
		{"pip parents", filepath.Join(".config", "pip", "pip.conf")},
		{"uv parents", filepath.Join(".config", "uv", "uv.toml")},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			home := t.TempDir()
			h := newSecureTestHome(t, home)
			if err := h.ensureParent(tc.path, 0o700); err != nil {
...
func TestSecureUserFile_ParentRefusals(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			path: filepath.Join(".config", "pip", "pip.conf"),
		},
		{
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			tc.seed(t, home)
			h := newSecureTestHome(t, home)
			if err := h.ensureParent(tc.path, 0o700); !errors.Is(err, ErrTargetUnusable) {
	...
	home := t.TempDir()
	h := newSecureTestHome(t, home)
	if err := h.ensureParent(filepath.Join("..", "outside", "pip.conf"), 0o700); !errors.Is(err, ErrTargetUnusable) {
...
func TestSecureUserFile_ParentHardeningFailureRemovesNewDirectory(t *testing.T) {
	home := t.TempDir()
	h := newSecureTestHome(t, home)
	h.applyMetadata = func(*secureUserHome, *os.File, os.FileMode, bool) error {
		return errors.New("hardening failed")
	}
	path := filepath.Join(".config", "pip", "pip.conf")
	if err := h.ensureParent(path, 0o700); err == nil {
	...
	h.applyMetadata = func(*secureUserHome, *os.File, os.FileMode, bool) error { return nil }
	if err := h.ensureParent(path, 0o700); err != nil {
...
func TestSecureUserFile_ParentSwapDuringCreationRejected(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(outside, "pip")); !os.IsNotExist(err) {
...
func TestSecureUserFile_SymlinkPolicy(t *testing.T) {
	t.Run("relative in-home leaf", func(t *testing.T) {
		...
		leaf := filepath.Join(home, "dotfiles", "pip.conf")
		if err := os.WriteFile(leaf, []byte("before"), 0o600); err != nil {
			t.Fatal(err)
		}
		if err := os.Symlink(filepath.Join("dotfiles", "pip.conf"), filepath.Join(home, "pip.conf")); err != nil {
			t.Fatal(err)
		}
		f := openSecureTestFile(t, newSecureTestHome(t, home), "pip.conf")
		...
		if info, err := os.Lstat(filepath.Join(home, "pip.conf")); err != nil || info.Mode()&os.ModeSymlink == 0 {
	...
	t.Run("relative in-home clear purges resolved backups", func(t *testing.T) {
		...
		if err := os.Symlink(filepath.Join("dotfiles", "credentials"), filepath.Join(home, ".netrc")); err != nil {
			t.Fatal(err)
		}
		f := openSecureTestFile(t, newSecureTestHome(t, home), ".netrc")
	...
	t.Run("deep chain", func(t *testing.T) {
		home := t.TempDir()
		for i := 0; i <= secureUserFileMaxSymlinkDepth; i++ {
...
func TestSecureUserFile_SnapshotBackupRotationAndCleanup(t *testing.T) {
	...
	for i := 0; i < secureUserFileMaxBackups+3; i++ {
	...
	if len(backups) == 0 || len(backups) > secureUserFileMaxBackups {
		t.Fatalf("backup count = %d, want 1..%d", len(backups), secureUserFileMaxBackups)
	}
