...
// resolveDerived vets a corroborator a ladder derives beside an accepted
// candidate (a sidecar, a manifest). Its parent is the candidate's
// EvalSymlinks result, already link-free and guarded, so only the final
// component can be a link — and a linked corroborator is rejected unread,
// seen with Readlink rather than followed, since its target could lie
// anywhere. What remains is a plain path, guarded like the candidate itself;
// an absent one passes here for the caller's Stat to decide.
func resolveDerived(exec executor.Executor, homeDir string, skipper *tcc.Skipper, path string) (string, bool) {
	...
	if newCandidateGuard(exec, homeDir, skipper).protected(path) {
		return path, false
	}
	return path, true
}
