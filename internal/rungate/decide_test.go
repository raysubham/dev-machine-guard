...
func TestDecidePrecedence(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			in:         Inputs{ForceScan: true, Directive: skipDirective, Now: now},
			wantSkip:   false,
			wantReason: "forced",
		},
		{
			name:       "kill switch never skips",
			in:         Inputs{KillSwitch: true, Directive: skipDirective, Now: now},
			wantSkip:   false,
			wantReason: "kill_switch",
		},
		{
			name:       "skip directive obeyed",
			in:         Inputs{Directive: skipDirective, Now: now},
			wantSkip:   true,
			wantReason: "directive:not_due",
		},
		{
			name:       "full directive obeyed",
			in:         Inputs{Directive: fullDirective, Now: now},
			wantSkip:   false,
			wantReason: "directive:interval_elapsed",
		},
		{
			name:       "unknown future mode degrades to full, never skip",
			in:         Inputs{Now: now, Directive: &Directive{Mode: "partial", Reason: "phased_rollout", GatingEnabled: true}},
			wantSkip:   false,
...
func TestDecideSkipCarriesNextEligible(t *testing.T) {
	...
	dec := Decide(Inputs{Directive: d, Now: now})
...
func TestDecideOfflineFallback(t *testing.T) {
	...
	state := func(mutate func(*heartbeat.RunGate)) *heartbeat.RunGate {
	...
	tests := []struct {
		...
		{name: "gating was off at last contact", st: state(func(s *heartbeat.RunGate) { s.GatingEnabled = false }), wantSkip: false, wantReason: "offline_fail_open"},
		{name: "no cached interval", st: state(func(s *heartbeat.RunGate) { s.EffectiveIntervalMinutes = 0 }), wantSkip: false, wantReason: "offline_fail_open"},
		{name: "never completed a run", st: state(func(s *heartbeat.RunGate) { s.LastFullRunAt = 0 }), wantSkip: false, wantReason: "offline_fail_open"},
		{name: "never checked in", st: state(func(s *heartbeat.RunGate) { s.DirectiveFetchedAt = 0 }), wantSkip: false, wantReason: "offline_fail_open"},
		{name: "fresh cache within interval skips", st: state(nil), wantSkip: true, wantReason: "offline_cache_skip"},
		{name: "interval elapsed locally runs", st: state(func(s *heartbeat.RunGate) { s.LastFullRunAt = nowUnix - int64(interval)*60 }), wantSkip: false, wantReason: "offline_fail_open"},
		{
			name: "stale cache beyond max(floor, 2x interval) runs",
			st: state(func(s *heartbeat.RunGate) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			dec := Decide(Inputs{State: tt.st, Now: now})
			if dec.Skip != tt.wantSkip || dec.Reason != tt.wantReason {
				t.Fatalf("Decide() = (skip=%v, reason=%q), want (skip=%v, reason=%q)",
					dec.Skip, dec.Reason, tt.wantSkip, tt.wantReason)
			}
