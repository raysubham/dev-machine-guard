...
- **WSL distributions are scanned, not just counted.** The Windows agent already reported registered distributions and whether one was running under `device.wsl`; each distro now also carries its Lxss GUID and `DefaultUid`, and a new `wsl_scan` phase runs the Linux agent inside every *running* distribution over `/mnt/c`, so the packages, configs and credentials inside a developer's Linux environment land as their own device record. A guest is identified by `uuidv5(host serial | distro id)` because a distro's own identity is unusable — it inherits the host's hostname and a minimal or WSL1 distro has no `/etc/machine-id`, so every such distro on a fleet would collide onto one record — and it gates on the tenant's cadence like any other device. A stopped distro is never started, a root-only distro (`DefaultUid` 0) is skipped, and the host never waits for results: the guest owns its own upload. The Linux binary ships as a third MSI component beside the agent, per architecture (Windows on ARM runs an aarch64 WSL2 kernel), costing ~11-12 MB per MSI on machines that may have no WSL; Windows never executes it. Distro scanning is tenant-wide and **fails closed** — it stays off unless the run-config check-in returns `wsl_directive.enabled`, and an absent block, an older backend, a failed check-in or a `--force-scan` bypass all leave it off. `STEPSEC_FORCE_WSL_SCAN=1` is the only local escape. Host-side detection is unchanged and stays on.
- **Insomnia credentials in the credential-location inventory**: a new `insomnia` source (category `api_clients`) reads the fixed set of Insomnia database files under the platform data root, or under `INSOMNIA_DATA_PATH`, and reports one finding per file. It counts credential material across every retained JSON-line record — request, folder and websocket auth fields, credential headers, gRPC metadata, environment variables under credential names, secret-typed variables classified by their storage envelope, OAuth tokens, client-certificate passphrases and gzip-compressed request history — plus Insomnia's own account and provider stores. Deleted and superseded records count too, because their bytes stay in the file until compaction, and per-file record and expansion bounds report as capped.
- **Nine more AI tools in the inventory**: Grok Build, Kimi Code, Muse Code, Hermes Agent and Oh My Pi as agent CLIs, and Kiro (Amazon) as an IDE, a CLI and a skills root. Every one shares its binary name with an unrelated tool, so identity is proven per install channel before a row is reported — npm manifest, vendor install directory, Homebrew keg or cask, winget, pacman, mise, a uv or pipx venv, the `dev.kiro.desktop` bundle id, `HKCU\SOFTWARE\Kiro\CLI` — and versions come only from on-disk evidence. Nothing is executed, and no agent config, auth, session or log file is read. The matching skills roots and project markers are registered alongside, and Windows directory junctions now fold like symlinks so a linked corroborator is rejected rather than followed. `SCAN_COVERAGE.md` records the layouts deliberately left out.
- **Agent Plugins inventory for Claude Code and Codex**: reads installed plugins and marketplaces from local state, including local, Git, hosted and account-managed sources. Reports installation and enablement separately, source and version metadata, and supplied skills, commands and MCP servers. Standalone Claude commands and Claude's recorded skill-use counts are included. The scanner does not run either CLI, and incomplete reads do not claim that a plugin was removed.
- **PyPI secure-registry device policy**: a `package_config#pypi` lane points pip and uv at the tenant's secure registry — `index-url` in every trusted user-tier pip config path pip itself would read, the index in `uv.toml`, and one `.netrc` entry carrying the device token for both. Like the npm lane it runs both enforcement channels (`dmg` writes and verifies, `mdm` only observes), reports an effective status that names the override when an environment variable (`PIP_INDEX_URL`, `UV_INDEX*`, `NETRC`, …) outranks the file, and never puts a token in a report or a log.
- **Go secure-registry device policy**: the same lane for `package_config#go`, managing `GOPROXY` in the resolved user's Go env file plus the shared `.netrc` credential. Ownership markers record which channel wrote the block, so a `dmg` run refuses a target an MDM already owns instead of fighting it, and CRLF and Windows `_netrc` spellings round-trip unchanged.
- **Managed npm settings in device policy**: the `package_config#npm` lane now accepts a settings map alongside — or instead of — the StepSecurity registry pair, rendering sorted `key=value` lines into the managed block and treating `registry` as an ordinary last-wins setting. A settings-only policy restores any user registry line the lane had previously prefixed, and reports carry a `settings_status`. A StepSecurity-only policy still writes byte-identically to before.
- **Tenant switch for credential scanning**: the run-config check-in now carries `scanners.credentials.enabled`, and a device whose tenant has turned it off skips the phase entirely — no detector is built, no environment probe or file read happens, no phase is recorded, and `credential_scan` is absent from the upload. The last explicit answer is cached in `last-run.json`; a missing, malformed or failed response keeps it, and with no usable cache the agent scans. `--force-scan`, `STEPSEC_FORCE_SCAN` and `STEPSEC_DISABLE_RUN_GATE` now bypass cadence only — the check-in still runs, so a forced run cannot defeat a tenant's off.
...
- **Global Node package roots follow the developer's home on macOS**, resolved from the console user rather than the service account, so a root LaunchDaemon no longer looks for nvm, pnpm and yarn globals under `/var/root`. Windows keeps `USERPROFILE` and Linux keeps `HOME`.
